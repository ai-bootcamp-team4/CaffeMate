...
export function WorkflowProgressView({ progress, compact = false }: {
  ...
  return <section className="workflow-progress-view" data-compact={compact} aria-label="분석 진행 상황" aria-live="polite">
    ...
    {!compact && progress.stages.length > 0 && <ol className="workflow-progress-view__stages">
      {progress.stages.map((stage) => <li key={stage.stage_run_id} data-state={stage.status}>
        <span aria-hidden="true" />
        <span>
          {workflowStageLabel(stage.stage_code)}
          {stage.status === 'SKIPPED' ? ' · 이번 재계산에서는 생략' : ''}
        </span>
      </li>)}
    </ol>}
  </section>
