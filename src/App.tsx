...
type PropertyRecalculation =
  | { mode: 'LIVE'; application: PropertyTermsApplication; candidate: ResultCandidate }
...
function PreparationScreen({ client, projectId, candidate, selection, guide, busy, error, onRetry, onBack, onApply, onDocumentApplied }: { client: ControlApiClient; projectId: string; candidate: ResultCandidate; selection: CandidateSelection; guide: PreparationGuide | null; busy: boolean; error: string; onRetry: () => void; onBack: () => void; onApply: (terms: PropertyTermsInput) => Promise<PropertyRecalculation>; onDocumentApplied: () => Promise<void> }) {
...
function ResultScreen({ client, project, initialResult }: { client: ControlApiClient; project: Project; initialResult: ResultView }) {
  ...
  if (selection && preparationOpen) return <PreparationScreen client={client} projectId={project.project_id} candidate={candidates.find((candidate) => candidate.candidate_id === selection.candidate_id) ?? activeCandidate} selection={selection} guide={preparationGuide} busy={preparationBusy} error={preparationError} onRetry={() => void loadPreparationGuide(selection)} onBack={() => { setPreparationOpen(false); window.scrollTo({ top: 0 }) }} onApply={applyPropertyTerms} onDocumentApplied={refreshAfterDocument} />
