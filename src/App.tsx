...
const procedureTitles: Record<ProcedureType, string> = {
  BUSINESS_REGISTRATION: '사업자등록',
  FOOD_SERVICE_REPORT: '휴게음식점 영업신고',
  FACILITY_REQUIREMENTS: '시설 기준 확인',
  HYGIENE_EDUCATION: '위생교육',
  SIGNAGE: '간판 신고 확인',
  FIRE_SAFETY: '소방 안전 확인',
}

function procedureStatus(status: PreparationGuide['procedures'][number]['status']) {
  return {
    OK: '공식 자료 확인',
    PARTIAL: '일부 자료 확인',
    STALE: '기준일 재확인 필요',
    NOT_FOUND: '공식 자료 추가 확인 필요',
    ERROR: '공식 자료를 불러오지 못함',
  }[status]
}
...
function PreparationScreen({ candidate, selection, guide, busy, error, onRetry, onBack }: { candidate: ResultCandidate; selection: CandidateSelection; guide: PreparationGuide | null; busy: boolean; error: string; onRetry: () => void; onBack: () => void }) {
  const jurisdiction = guide?.jurisdiction_display_name
  return <div className="shell min-h-dvh"><header className="topbar"><a className="wordmark" href="#preparationTop">CaffeMate</a><div className="topbar__meta"><Badge tone="success">검토 대상 선택됨</Badge></div></header><main className="page preparation-page" id="preparationTop"><header className="preparation-hero"><div><p className="candidate-picker__count">선택한 창업안</p><h1>{candidate.display_name}의 실제 준비를 시작해요</h1><p>필요한 자료와 공식 절차를 하나씩 확인하면, 임시 계산값을 실제 점포 조건으로 바꿀 수 있어요.</p></div><button className="btn btn--accent" type="button" onClick={onBack}>결과 비교로 돌아가기</button></header><div className="preparation-layout"><section className="preparation-main" aria-labelledby="evidenceChecklistTitle"><article className="surface"><div className="surface__head"><h2 id="evidenceChecklistTitle">먼저 준비할 자료</h2><p>자료를 모두 모은 뒤 시작할 필요는 없어요. 확보되는 순서대로 추가하면 판단을 다시 계산합니다.</p></div>{selection.required_evidence.length ? <ol className="preparation-checklist">{selection.required_evidence.map((item, index) => <li key={item.code}><span className="preparation-checklist__number">{index + 1}</span><div><strong>{item.title}</strong><p>{item.reason}</p></div></li>)}</ol> : <p>현재 결과에서 별도로 지정된 필수 자료는 없어요. 아래 공식 절차와 실제 점포 조건부터 확인해 주세요.</p>}</article><article className="surface" aria-labelledby="officialProcedureTitle"><div className="surface__head"><h2 id="officialProcedureTitle">지역별 공식 창업 절차</h2><p>{jurisdiction ? `${jurisdiction} 기준으로` : '선택 지역 기준으로'} 신고·교육·시설 확인 순서를 안내합니다. 실제 신청과 제출은 자동으로 진행하지 않아요.</p></div>{busy && <div className="preparation-loading" role="status"><span aria-hidden="true" /><p>공식 준비 절차를 확인하고 있어요.</p></div>}{!busy && error && <div className="warning-box" role="alert"><span aria-hidden="true">!</span><div><strong>후보 선택은 저장되었어요.</strong><p>{error}</p><button className="btn btn--accent" type="button" onClick={onRetry}>공식 절차 다시 확인하기</button></div></div>}{!busy && guide && <div className="procedure-list">{guide.procedures.map((procedure) => <section className="procedure-row" key={procedure.procedure_type}><div className="procedure-row__head"><h3>{procedureTitles[procedure.procedure_type]}</h3><Badge tone={procedure.status === 'OK' ? 'success' : 'warning'}>{procedureStatus(procedure.status)}</Badge></div>{procedure.steps.length ? <ol>{procedure.steps.map((step) => <li key={`${step.step_order}-${step.title}`}><strong>{step.title}</strong><span>{step.authority} · 기준일 {new Intl.DateTimeFormat('ko-KR').format(new Date(`${step.source_date}T00:00:00`))}</span></li>)}</ol> : <p>확인된 공식 단계가 아직 없어요. 관할 기관 자료를 직접 확인해 주세요.</p>}</section>)}</div>}</article></section><aside className="preparation-side"><article className="surface"><div className="surface__head"><h2>다음에 달라지는 것</h2></div><ul className="plain-list"><li><div><strong>점포 조건 반영</strong><p>{selection.property_intake_enabled ? '보증금·월세·권리금과 면적을 실제 값으로 바꿀 수 있어요.' : '현재는 점포 조건 입력을 지원하지 않아요.'}</p></div></li><li><div><strong>견적과 문서 반영</strong><p>{selection.document_intake_enabled ? '인테리어·장비 견적과 계약 자료를 추가해 비용과 위험을 다시 계산할 수 있어요.' : '현재는 문서 입력을 지원하지 않아요.'}</p></div></li><li><div><strong>결정은 직접</strong><p>계약, 결제, 대출 실행과 최종 창업 결정은 자동으로 진행하지 않아요.</p></div></li></ul></article><article className="decision-note"><strong>현재 완료된 것</strong><span>{candidate.display_name}을 검토 대상으로 선택했어요. 아직 창업 준비나 계약이 완료된 것은 아니에요.</span></article></aside></div></main><footer className="footer"><strong>CaffeMate</strong><span>카페 창업, 감이 아닌 데이터로 시작하세요.</span></footer></div>
}
