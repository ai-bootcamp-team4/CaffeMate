...
- 결과 화면은 탭형 대시보드가 아니라 `결론 → 후보 비교 → 판정 이유 → 재무·근거와 실제값 정밀화 행동 → 참고 상권 정보 → 외부 확인 → 실제 창업 절차·인간 결정 경계` 순서의 단일 읽기 흐름으로 제공한다. 자연어 채팅은 화면 우하단의 작은 고정 런처로 항상 접근 가능하게 두고, 사용자가 누를 때만 대화·입력 패널을 펼친다.
