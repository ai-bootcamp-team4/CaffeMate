...
- 결과 화면은 탭형 대시보드가 아니라 `결론 → 후보 비교 → 판정 이유 → 재무·근거와 실제값 정밀화 행동 → 참고 상권 정보 → 외부 확인 → 실제 창업 절차·인간 결정 경계` 순서의 단일 읽기 흐름으로 제공한다. 자연어 채팅 입력은 이 화면 하단에 고정한다.
...
- 결과 화면은 먼저 확정된 결과와 근거를 읽는 작업 공간이며, 자연어 채팅 입력은 결과 화면을 스크롤하는 동안 항상 접근할 수 있도록 화면 하단에 고정한다. 설명 질문과 조건 변경 요청을 위한 별도 모드 전환은 두지 않는다.
