...
flowchart TD
    ...
    result --> chat[하단 자연어 채팅]
    chat -->|일반 질문| explain[결과 근거 설명]
    chat -->|조건 변경| confirm[변경안 확인]
    confirm --> result
    result --> finance[비용·계산 근거]
    finance --> refine[실제 숫자로 정밀화]
    refine --> recompute[충돌 확인과 재계산]
    recompute --> result
    result --> procedure[외부 확인·공식 창업 절차]
```
...
온보딩 안에서 `다음`·`이전`·단계 이동·수정·선택처럼 같은 입력 화면의 문맥을 바꾸는 조작은 현재 스크롤 위치를 강제로 초기화하지 않는다. 프로젝트 목록→온보딩, 온보딩→분석/결과처럼 화면 자체가 전환될 때만 새 화면의 시작 위치로 이동한다.
...
개인 가맹 모집이 별도 공식 근거로 확인되어 이미 후보가 된 브랜드에는 공정거래위원회의
신고연도별 창업금액 구조화 자료를 재무 근거로 추가할 수 있다. 같은 브랜드·신고연도의 가맹금,
교육비, 가맹사업자 보증금, 기타 초기금액과 공표 합계가 모두 존재하고 구성항목 합계가 공표
합계와 일치할 때만 그 합계를 `FRANCHISE_INITIAL_FEES`의 `EVIDENCED_FACT`로 사용한다. 이 값은
해당 신고연도의 공식 비용 자료이지 최신 정보공개서 문서 버전, 현재 가맹 모집 상태 또는 특정
주소의 출점 승인을 뜻하지 않는다. 최신 정보공개서 등록번호·등록일과 원문이 확인되지 않았으면
그 문서 요구사항은 계속 미확인으로 남긴다.
...
독립카페 모델의 월 점유비는 실제 점포가 정해지기 전까지 한국부동산원의 상업용부동산
임대동향 지역 benchmark를 등록 면적·보증금 가정과 결합한 추정값으로 사용할 수 있다. 원자료의
환산임대료에는 보증금의 환산 비용이 포함되고 관리비는 제외되므로, Control API는 등록 보증금에
해당 지역 전환율을 적용한 월 환산분을 빼서 추정 월세를 만들고 등록 관리비 비율을 더해
`MONTHLY_OCCUPANCY`를 계산한다. 이 값은 항상 `BENCHMARK`로 표시하고 사용한 면적·보증금·관리비
가정, 지역 범위, 기준분기, 1층 환산 기준과 변환식을 함께 공개한다. 특정 매물의 실제 월세나
관리비로 표현하지 않으며, 사용자가 실제 점포·임대차의 월세와 관리비를 확인하면 그 값이 즉시
benchmark를 대체한다. 지원 상권과 정확히 연결되지 않은 행정동을 유사 상권으로 임의 매칭하지
않고, 현재 지원 가능한 상위 지역 범위만 명시적으로 사용한다.

사용자가 실제 점포 조건을 입력하면 Control API는 비용뿐 아니라 주소·면적·층도 하나의 현재
`PropertyContext`로 보존하고 이후 재계산에 그대로 이어받는다. 보증금·월세·관리비·권리금만
현재 재무 계산의 대응 항목을 교체하며, 주소·면적·층은 향후 공간·건물 근거 조회를 위한 사용자
확인 사실이다. 주소 문자열이나 면적·층 입력만으로 법정동 코드, 건물 사실, 인허가 적합성 또는
프랜차이즈 본사의 후보 주소 승인을 만들어내지 않는다.

매출 자료가 없으면 예상매출 대신 사용자 가정 기반 손익분기 매출과 필요한 주문 수를 계산한다.
프랜차이즈 로열티는 계약 의미에 따라 분리한다. 월 정액 로열티는 월 고정비에 포함하지만,
매출의 일정 비율로 부과되는 로열티는 월 고정비로 환산하지 않고 `sales_royalty_bps`라는
매출연동 변동비율로 보존한다. 이 비율은 등록 공헌이익률에서 차감한 유효 공헌이익률을 만든 뒤
손익분기 매출과 필요 주문 수에 적용한다. 숫자와 단위가 확인되지 않은 로열티는 0으로 간주하지
않으며, 유효 공헌이익률이 0 이하가 되면 손익분기 계산은 `UNKNOWN`으로 닫는다. 프랜차이즈
계약·정보공개서에서 사용자가 확인한 명시적 퍼센트 또는 bps 로열티는 일반 브랜드 profile보다
우선하고, 재계산 결과는 기준 공헌이익률·총 변동비율·유효 공헌이익률과 해당 decision input의
provenance를 함께 공개한다.
...
개인카페 모델의 월 인건비에는 대표자 본인 노동과 유급 직원 노동을 섞지 않는다. 각 seed는
별도의 `paid_staff_fte` low·base·high 가정을 가지며, Control API는 평가일에 유효한 공식
최저임금의 월 209시간 환산액을 해당 유급 직원 FTE에 적용해 법정 하한을 계산한다. 이 하한이
등록 인건비 가정보다 높은 지점만 `BENCHMARK`로 올리고, 이미 더 높은 등록 가정은 낮추지 않는다.
최저임금 benchmark는 실제 급여명세가 아니며 연장·야간·휴일 가산수당과 사용자 부담 사회보험료는
포함하지 않는다. 사업주 사회보험 부담은 `MONTHLY_LABOR`에 섞지 않고 별도
`MONTHLY_EMPLOYER_ONCOST`로 계산한다. 2026년에는 국민연금 4.75%, 건강보험·장기요양 합계
4.0674%, 실업급여 0.9%, 150인 미만 고용안정·직업능력개발 0.25%의 사업주분만 적용한다.
계산 기준은 같은 최저임금 월환산액과 `paid_staff_fte`로 만든 직원 급여 하한이며 대표자 노동은
제외한다. 업종별 산재보험 요율과 확인되지 않은 보험료 상·하한, 면제, 지원금은 추정하지 않고
제외항목으로 공개한다. 최저임금 또는 유효 사회보험 schedule이 없으면 이 비용과 월 손익분기는
0원이 아니라 `UNKNOWN`이다. 실제 급여 계획이나 확인 문서가 생기면 그 값을 더 높은 우선순위로
사용한다.
...
자료 부족 자체는 `제외` 사유가 아니다. 확인된 Hard Constraint 위반만 제외 근거가 된다. 특정 후보 주소의 프랜차이즈 출점 승인처럼 CaffeMate가 권위 있게 닫을 수 없는 항목은 경제 검토 상태와 분리한 `외부 확인 필요`로 유지하며, 그 사실만으로 경제 검토를 `조건부 검토`로 낮추지 않는다.

`조건부 검토` 후보도 결과 순위에 포함할 수 있다. 이때 순위는 확정 경제성 순위가 아니라 현재 근거에서의 `다음 검토 우선순위`이며, 상태·누락값·판단 영향이 순위 바로 옆에 표시돼야 한다. Control API는 Gate와 순위를 계산한 시점의 결정 입력·reason code·순위 factor를 함께 고정해 공개하고, 프론트엔드는 이를 다시 계산하지 않는다.
...
- 결과 화면은 탭형 대시보드가 아니라 `결론 → 후보 비교 → 판정 이유 → 재무·근거와 실제값 정밀화 행동 → 참고 상권 정보 → 외부 확인 → 실제 창업 절차·인간 결정 경계` 순서의 단일 읽기 흐름으로 제공한다. 이 읽기 순서를 바꾸지 않는 작은 섹션 바로가기 바를 제공해 각 결과 구간으로 직접 이동할 수 있게 한다. 자연어 채팅은 화면 우하단의 작은 고정 런처로 항상 접근 가능하게 두고, 사용자가 누를 때만 대화·입력 패널을 펼친다.
- 결과의 미확정 항목은 하나의 `확인 필요`로 합치지 않는다. 사용자·문서 입력으로 CaffeMate가 다시 계산할 수 있는 항목, 외부 주체만 최종 확인할 수 있는 항목, 계산에 쓰지 않은 참고 자료를 서로 분리한다.
- 결과 카드에는 현재 판단, 근거, 알려지지 않은 값, 비용 범위와 위험을 표시한다. 판단 반전 조건은 별도 섹션을 만들지 않고 현재 Result 계약에 유지하여 자연어 설명과 향후 what-if 기능에서 사용한다.
...
- 결과 화면은 먼저 확정된 결과와 근거를 읽는 작업 공간이며, 자연어 채팅은 결과 화면을 스크롤하는 동안 항상 접근할 수 있도록 작은 고정 런처로 유지한다. 기본 상태에서는 결과 내용을 가리지 않으며, 사용자가 런처를 누르면 같은 위치에서 대화·입력 패널을 펼친다. 설명 질문과 조건 변경 요청을 위한 별도 모드 전환은 두지 않는다.
...
- 자연어 입력이 일반 결과 질문인지 조건 변경 요청인지는 백엔드가 판별한다. 일반 질문은 반환된 설명만 표시하며 Founder State, Venture State, 후보, 순위와 결과를 바꾸지 않는다.
- 백엔드가 조건 변경 요청으로 표시한 경우에만 같은 사용자 문장으로 feedback preview를 만들고 확인 UI를 연다. 프론트엔드는 문자열 규칙으로 조건 변경 여부를 판별하거나 변경 operation을 생성하지 않는다.
- 조건 변경 preview는 변경 전후 차이와 적용 버튼을 보여주고 사용자가 확인한 뒤 기존 feedback confirm API로만 반영한다.
- 확인 전에는 기존 State와 결과를 바꾸지 않는다.
- 반영 후 영향받는 검색·후보·계산만 재실행한다.

## 13. 실제 숫자로 정밀화

사용자가 주력 또는 비교 후보를 선택하면 현재 판단을 실제 조건으로 좁힐 자료를 추가할 수 있다. 선택 응답은 모든 자료를 고정 `필수` 체크리스트로 만들지 않고, 현재 unresolved decision input을 실제로 교체할 수 있는 `CaffeMate에서 보강 가능` 항목과 본사·관할기관 등만 결정할 수 있는 `외부 확인 필요` 항목을 분리한다. 이미 실제 값으로 해소된 항목이나 현재 판단에 영향을 주지 않는 자료는 다시 요구하지 않는다.

정밀화의 기본 원칙은 첫 결과 화면을 메인 작업 공간으로 두는 것이다. 사용자는 별도의 `검증 여정`으로 들어가지 않고 재무·근거의 개별 입력 옆 행동에서 필요한 숫자만 정밀화한다.

- 지역 임차비 참고값은 `실제 매물로 바꾸기`로 들어가 `직접 입력` 또는 `파일로 불러오기`를 선택한다. 파일 경로는 점포 매물 자료나 상가 임대차계약서에서 주소·면적·층·보증금·월세·관리비·권리금을 추출해 같은 점포 입력 폼에 채우며, 사용자가 빠진 값을 확인한 뒤 한 번의 property apply/recompute로 반영한다.
- 장비비·인테리어비·가맹 초기비용·로열티 등 문서로 교체 가능한 입력은 해당 `resolution_action`과 허용 문서 종류에 맞는 정밀화 화면을 연다.
- 정밀화 화면은 선택 후보, 바꿀 입력의 현재 값·provenance·영향 계산, 실제 입력 수단, 재계산 결과와 causal `DecisionDelta`, 결과로 돌아가기를 표시한다.
- 외부 확인과 공식 창업 준비 절차는 정밀화 화면의 단계가 아니다. 첫 결과 화면 하단에서 별도로 확인한다.
- 실제값 반영 전에는 기존 State·비용·Gate·순위를 변경하지 않는다. 반영 뒤에는 Control API가 만든 authoritative result만 표시하며 프론트엔드는 재무·Gate·rank를 다시 계산하지 않는다.
...
자료가 들어오면 사실 추출, 기존 사실 간 충돌 확인, 재무·위험 재계산, 판단 변화 설명 순서로 처리한다. 사용자가 확정한 현재 후보의 문서 값은 Control API의 단일 finance resolver를 통해 등록 가정보다 우선한다. 현재 연결 범위에서는 임대차·매물의 보증금·권리금·월세+관리비, 인테리어 견적 총액, 장비 견적 총액을 각각 대응하는 비용 항목으로만 반영한다. 문서 종류가 다른 `QUOTE_TOTAL`은 서로 다른 비용 의미이므로 충돌로 취급하지 않는다. 같은 문서 종류·같은 Claim의 확인값이 충돌하면 임의로 최신값을 고르지 않고 해당 비용을 `UNKNOWN`으로 둔다. 핵심 자료가 부족해도 빈 화면으로 끝내지 않고 현재 판단을 바꿀 수 있는 다음 입력과 외부에서만 해소 가능한 확인을 구분해 제공한다.
...
`VITE_UI_ONLY=true` 개발 모드는 백엔드 연결 완료 여부와 무관하게 전체 예정 UX surface를 검토하기 위한 fixture를 제공한다. 실제 매물, 장비 견적, 인테리어 견적, 가맹비·교육비·보증금, 로열티 문서 정밀화 행동을 모두 노출하며 아직 production 재계산 계약이 연결되지 않은 항목은 개발 미리보기임을 표시한다. 이 fixture는 production capability를 주장하지 않는다.

## 14. 외부 확인·창업 절차와 인간 결정 경계

외부 주체만 확정할 수 있는 본사 승인·관할기관 확인과 공식 창업 준비 절차는 첫 결과 화면의 하단에 둔다. 창업 준비 절차는 사용자가 요청할 때만 lazy-load하며, 내부적으로 필요한 CandidateSelection은 별도 검증 단계처럼 노출하지 않는다.
...
- 새 실제 문서는 등록 가정을 대체하고 영향받은 계산만 갱신한다. 같은 문서 의미 범위의 서로 다른 사용자 확인값만 충돌로 처리한다.
- 재계산 결과는 바뀐 decision input의 값·provenance·영향 계산, reason code 증감과 실제 Gate 상태 전이를 `decision_delta`로 설명한다.
