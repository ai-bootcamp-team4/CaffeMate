...
- Evidence Freeze에 포함된 `PRIMARY_OFFICIAL` 근거는 후보 응답의 `official_documents`로 투영한다. 각 항목은 출처 제목·원문 주소·기준일·문서 revision·인용문·Evidence 참조와 실제 후보 판단 사용 여부를 보존한다. 현재 후보에 필요한 창업 절차·계약 확인·정보공개서 범주에 수용된 근거가 없으면 `official_document_gaps`에 명시하고 Agent가 빈 내용을 보충하지 않는다.
...
건수로 세지 않는다. 비용 입력 우선순위는 현재 선택 점포의 직접 입력 → 사용자 확인 문서 → 수용된 외부 근거 → 등록 모델 임시값이다.
임시값과 같은 항목의 실제 자료가 들어오면 실제 자료가 대체하며, 둘을 충돌로 오인하지 않는다. 사용자 문서는 문서 종류와 Claim의 명시적 매핑이 있을 때만 재무 입력이 된다. 현재 매핑은 임대차·매물의 보증금/권리금/월세+관리비, 인테리어 `QUOTE_TOTAL`, 장비 `QUOTE_TOTAL`이다. 인테리어와 장비처럼 문서 종류가 다르면 같은 `QUOTE_TOTAL` 이름을 공유해도 서로 충돌하지 않는다. 같은 문서 종류·같은 Claim의 열린 충돌은 자동 선택하지 않고 해당 비용을 `UNKNOWN`으로 전파한다.
