...
현재 production connector 범위는 [Production Capability](./mcp-production-capabilities.json)에 고정한다. `resolve_area`는 행정안전부 도로명주소 검색 API와 버전 고정 법정동 목록을 사용한다. `get_area_profile`과 `search_cafe_observations`는 승인된 BigQuery grounding snapshot만 조회한다. `get_property_reference`는 한국부동산원 상업용부동산 임대동향의 승인된 BigQuery snapshot에서 요청 행정코드의 상위 시도와 기준일에 맞는 최신 임대료·전환율 benchmark만 반환한다. 현재 connector는 상권명 유사 매칭이나 실제 매물 조회를 하지 않으며 `PARENT_REGION` 범위를 명시한다. Control API만 이 benchmark를 등록 독립카페 모델의 면적·보증금·관리비 가정과 결합해 `BENCHMARK` 월 점유비로 변환할 수 있고, 사용자 확인 점포·임대차 값이 있으면 이를 대체한다. `get_cost_reference`는 전국 공통 비용 기준의 effective-date snapshot을 읽는 범용 read tool이다. 현재 production 범위는 `MINIMUM_WAGE`와 `EMPLOYER_SOCIAL_INSURANCE`다. 최저임금은 2026년 시간급 10,320원·월 209시간 환산 2,156,880원과 이미 고시된 2027년 시간급 10,700원·월 2,236,300원을 각 시행기간과 함께 고정한다. Control API는 이 값을 등록 개인카페 seed의 `paid_staff_fte` 가정에만 곱해 `MONTHLY_LABOR`의 법정 하한을 만들며, 더 높은 기존 인건비 가정을 낮추지 않는다. 대표자 노동시간과 연장·야간·휴일 가산수당은 이 임금 하한에 넣지 않는다. 2026 사업주 사회보험 schedule은 국민연금 4.75%, 건강보험·장기요양 합계 4.0674%, 실업급여 0.9%, 150인 미만 고용안정·직업능력개발 0.25%, 합계 996.74bps만 고정한다. Control API는 같은 직원 급여 하한에 이 요율을 적용해 별도 `MONTHLY_EMPLOYER_ONCOST`를 만들며 `MONTHLY_LABOR`를 변경하지 않는다. 산업분류가 필요한 산재보험과 확인되지 않은 보험료 상·하한, 면제, 지원금은 적용하지 않고 공개 제외항목으로 남긴다. 유효 최저임금 또는 사회보험 schedule이 없으면 사업주 부담을 0원으로 만들지 않고 `UNKNOWN`으로 닫는다. `retrieve_official_documents`는 서울 Vertex AI RAG Engine의 승인 official corpus만 조회한다. `list_franchise_universe`는 2026-08-23에 브랜드 공식 가맹 안내·창업상담 페이지를 확인한 snapshot만 반환한다. 이 값은 개인의 일반적인 가맹 문의 가능성을 뜻하며 특정 후보 지역의 출점 승인이나 정보공개서 완전성을 뜻하지 않는다. `get_franchise_disclosure`는 이 회사 공식 근거로 이미 `VERIFIED`인 내부 브랜드 id에 대해서만 공정거래위원회 브랜드 목록과 신고연도별 창업금액의 승인된 BigQuery snapshot을 정확한 브랜드·가맹본부 identity로 조회한다. 현재 연결 범위는 가맹금·교육비·가맹사업자 보증금·기타 초기금액과 공표 합계이며, 공표 합계와 구성항목이 모두 일치한 accepted Evidence만 Control API의 `FRANCHISE_INITIAL_FEES`를 교체할 수 있다. 이 신고연도 snapshot은 법적 정보공개서 등록번호·등록일·원문 버전이 아니므로 connector는 해당 문서 identity를 missing context로 남기고 `PARTIAL`로 반환한다. 공정위 등록 사실만으로 현재 개인 가맹 모집이나 후보 주소 출점 승인을 추론하지 않는다. 따라서 각 브랜드의 `area_availability_hq_confirmation`과 실제 최신 `franchise_disclosure` 문서 요구사항은 별도로 남는다. `retrieve_project_documents` 등 connector가 없는 tool은 MCP client에게 호출 가능하다고 표시하지 않는다.
