...
describe('local model-backed Agent executors', () => {
  it('pins Gemini 3.7 Flash configuration and exposes no tool surface to the model', () => {
    const { task } = completeFixture('PROPOSE_INDEPENDENT')

    const invocation = buildModelInvocation(task)

    expect(invocation.model).toBe('gemini-3.7-flash')
  ...
  it('adds the repair instruction only for a repair task', () => {
    ...
    const invocation = buildModelInvocation(repairTask)
  ...
  it('parses one JSON result and lets the deterministic dispatcher validate it', async () => {
    ...
    const dispatched = await dispatchAgentTask(task, createModelExecutors(client))
  ...
  it('rejects prose or Markdown instead of extracting JSON from it', async () => {
    ...
    const executor = createModelExecutors(client).INTENT_INTERPRETER
