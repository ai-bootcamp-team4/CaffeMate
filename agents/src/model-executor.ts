...
export interface AgentModelRepairContext {
...
export class AgentModelError extends Error {
  constructor(
    ...
    message: string,
    public readonly repairContext?: AgentModelRepairContext,
  ) {
    super(`${code}: ${message}`)
...
function parseJsonResult(text: string): AgentSemanticResult {
  ...
  try {
    ...
    throw new AgentModelError(
      ...
      {
        previousResponseText: text.slice(0, 65536),
        validatorErrors: [{
          code: 'MODEL_JSON_INVALID',
          json_pointer: '',
          message: 'model response must be exactly one JSON object',
        }],
      },
    )
  }
    ...
    throw new AgentModelError(
      ...
      'model response must be exactly one JSON object',
      {
        ...
        validatorErrors: [{
          ...
          json_pointer: '',
          message: 'model response root must be exactly one JSON object',
        }],
      },
    )
  }
  ...
  if (unexpected.length > 0) {
    throw new AgentModelError(
      ...
      {
        previousResponseText: text.slice(0, 65536),
        validatorErrors: unexpected.slice(0, 50).map((field) => ({
          code: 'MODEL_SEMANTIC_ENVELOPE_INVALID',
          json_pointer: `/${field}`,
          message: 'Runtime-owned fields must not be generated by the model',
        })),
      },
    )
  }
